...
public class Salon_Service extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jList1.setBorder(javax.swing.BorderFactory.createTitledBorder("Services matched:"));
...
//Connection Method
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        double currbill = Double.parseDouble(tf1.getText());
        double bookid = Double.parseDouble(tf4.getText());

        try{
            for(int i = 0; i <= index; i++){
                String item = jTable1.getValueAt(i, 0).toString();
                connect().executeUpdate("Insert into Salon_service_list(Bill_no, Name) values("+(currbill)+", '"+(item)+"');");
            }
            ResultSet rs = connect().executeQuery("Select SUM(Price) from Salon_service_info join Salon_service_list on Salon_service_info.Name = Salon_service_list.Name where Bill_no = "+(currbill)+";");
            rs.next();
            double tp = rs.getDouble("SUM(Price)");
            connect().executeUpdate("Insert into Salon_info(Bill_no, Booking_id, Total_price, Bill_date) values("+(currbill)+", "+(bookid)+", "+(tp)+", str_to_date('"+(dm)+"', '%d/%m/%Y'));");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try{
            ResultSet rs = connect().executeQuery("Select Name from Salon_service_info where Name like '"+(ts)+"';");
            while(rs.next()){
    ...
    public static void main(String args[]) {
        try {
            ...
            java.util.logging.Logger.getLogger(Salon_Service.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
